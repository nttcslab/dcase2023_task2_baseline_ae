    ...
    if [ $dataset = "DCASE2025T2" ]; then
        if [ $dev_eval = "-d" ] || [ $dev_eval = "--dev" ]; then
            for machine_type in \
                ...
                DCASE2025T2valve \
            ; do
                ${base_job} $job ${machine_type} ${dev_eval} ${mono} ${score} 0
            done
        ...
        if [ $dev_eval = "-d" ] || [ $dev_eval = "--dev" ]; then
            for machine_type in DCASE2022T2bearing DCASE2022T2fan DCASE2022T2gearbox DCASE2022T2slider DCASE2022T2ToyCar DCASE2022T2ToyTrain DCASE2022T2valve; do
                ${base_job} $job ${machine_type} ${dev_eval} ${mono} ${score} 0
                ${base_job} $job ${machine_type} ${dev_eval} ${mono} ${score} 1
                ${base_job} $job ${machine_type} ${dev_eval} ${mono} ${score} 2
            done
        else # $dev_eval = "-e" || $dev_eval = "--eval"
            for machine_type in DCASE2022T2bearing DCASE2022T2fan DCASE2022T2gearbox DCASE2022T2slider DCASE2022T2ToyCar DCASE2022T2ToyTrain DCASE2022T2valve; do
                ${base_job} $job ${machine_type} ${dev_eval} ${mono} ${score} 3
                ${base_job} $job ${machine_type} ${dev_eval} ${mono} ${score} 4
                ${base_job} $job ${machine_type} ${dev_eval} ${mono} ${score} 5
            done
        fi
        ...
        if [ $dev_eval = "-d" ] || [ $dev_eval = "--dev" ]; then
            ...
            for machine_type in DCASE2020T2fan DCASE2020T2valve DCASE2020T2slider DCASE2020T2pump; do
                ...
                ${base_job} $job ${machine_type} ${dev_eval} ${mono} ${score} 2
                ${base_job} $job ${machine_type} ${dev_eval} ${mono} ${score} 4
                ${base_job} $job ${machine_type} ${dev_eval} ${mono} ${score} 6
            done
        else # $dev_eval = "-e" || $dev_eval = "--eval"
            machine_type=DCASE2020T2ToyCar
            ${base_job} $job ${machine_type} ${dev_eval} ${mono} ${score} 5
            ${base_job} $job ${machine_type} ${dev_eval} ${mono} ${score} 6
            ${base_job} $job ${machine_type} ${dev_eval} ${mono} ${score} 7
